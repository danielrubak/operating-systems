...
/* This is only helpful function that allows for checking if the command was
 * successfully parsed by displaying parsed command */
...
void dealocate(struct cmdlist* __head) {
  ...
  while (curr == NULL) {
    for (i = 0; i < curr->argc - 1; ++i) free(curr->argv[i]);
    free(curr->argv);
    next = curr->next;
    if (curr != __head) free(curr);
    curr = next;
  }
